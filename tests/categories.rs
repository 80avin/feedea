...
async fn categories_crud_tree_reparent_and_remove_children() {
    ...
    let create_resp = post_category(
        &app,
        &cookie,
        &format!(r#"{{"name":"Rust","parent_id":"{tech_id}"}}"#),
    )
    .await;
    assert_eq!(create_resp.status(), StatusCode::OK);
    let body = create_resp.into_body().collect().await.unwrap().to_bytes();
    ...
    let create_resp = post_category(
        &app,
        &cookie,
        &format!(r#"{{"name":"Nightly","parent_id":"{rust_id}"}}"#),
    )
    .await;
    assert_eq!(create_resp.status(), StatusCode::OK);
    let body = create_resp.into_body().collect().await.unwrap().to_bytes();
    let nightly_id = serde_json::from_slice::<serde_json::Value>(&body).unwrap()["category_id"]
        .as_str()
        .unwrap()
        .to_string();
    ...
    assert_eq!(
        find_node(categories, &nightly_id).expect("Nightly should be in tree")["parent_id"],
        rust_id
    );
    ...
    let rename_resp = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header("content-type", "application/json")
                .header(axum::http::header::COOKIE, &cookie)
                .body(Body::from(r#"{"name":"Technology"}"#))
                .unwrap(),
        )
        .await
        .unwrap();
    ...
    let categories = tree["categories"].as_array().unwrap();
    assert!(
        find_node(categories, &tech_id).is_none(),
        "Tech should be gone"
    );
    ...
    assert_eq!(
        find_node(categories, &nightly_id).expect("Nightly should still exist")["parent_id"],
        rust_id
    );
    ...
    let delete_resp = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header("content-type", "application/json")
                .header(axum::http::header::COOKIE, &cookie)
                .body(Body::from(r#"{"remove_children":true}"#))
                .unwrap(),
        )
        .await
        .unwrap();
    ...
    assert!(
        !ids.contains(&nightly_id),
        "Nightly should be gone with remove_children: {ids:?}"
    );
}
...
async fn delete_category_with_remove_children_prunes_saved_articles() {
    ...
    assert_eq!(create_resp.status(), StatusCode::OK);
    let body = create_resp.into_body().collect().await.unwrap().to_bytes();
    let del_id = serde_json::from_slice::<serde_json::Value>(&body).unwrap()["category_id"]
        .as_str()
        .unwrap()
        .to_string();
        ...
        .oneshot(
            Request::builder()
                ...
                .body(Body::from(format!(
                    r#"{{"url":"{feed_url}","title":"Prune Feed","category_id":"{del_id}"}}"#
                )))
                .unwrap(),
        )
        .await
    ...
    assert_eq!(
        saved["total"], 0,
        "saved article must be pruned when its category is deleted"
    );
    ...
    assert_eq!(
        engine_count, 1,
        "article should survive as an orphan in the engine db"
    );
