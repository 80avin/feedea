...
async fn settings_defaults_and_patch_roundtrip() {
    ...
    assert!(
        settings["keep_articles_days"].is_null(),
        "keep_articles_days defaults to null"
    );
    ...
    assert_eq!(resp.status(), StatusCode::OK);
    assert_eq!(
        get_settings(&app, &cookie).await["sync_interval_minutes"].as_i64(),
        Some(15)
    );
    ...
    assert_eq!(resp.status(), StatusCode::OK);
    assert_eq!(
        get_settings(&app, &cookie).await["keep_articles_days"].as_i64(),
        Some(30)
    );
    ...
    assert_eq!(resp.status(), StatusCode::OK);
    assert!(
        get_settings(&app, &cookie).await["keep_articles_days"].is_null(),
        "null keep_articles_days keeps everything"
    );
    ...
    let wrong = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header(axum::http::header::COOKIE, &cookie)
                .body(Body::from(
                    r#"{"current_password":"wrong","new_password":"new-pass"}"#,
                ))
                .unwrap(),
        )
        .await
    ...
    let correct = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header(axum::http::header::COOKIE, &cookie)
                .body(Body::from(
                    r#"{"current_password":"test-pass","new_password":"new-pass"}"#,
                ))
                .unwrap(),
        )
        .await
    ...
    let new_login = app
        ...
        .oneshot(
            Request::builder()
                ...
                .uri("/api/login")
                .header("content-type", "application/json")
                .body(Body::from(r#"{"password":"new-pass"}"#))
                .unwrap(),
        )
        .await
        .unwrap();
