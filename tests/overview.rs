...
async fn overview_parent_card_includes_descendant_feed_counts() {
    ...
    let parent_id = serde_json::from_slice::<serde_json::Value>(&body).unwrap()["category_id"]
        .as_str()
        .unwrap()
        .to_string();
        ...
        .oneshot(
            Request::builder()
                ...
                .body(Body::from(format!(
                    r#"{{"name":"Child","parent_id":"{parent_id}"}}"#
                )))
                .unwrap(),
        )
        .await
    ...
    assert_eq!(create_resp.status(), StatusCode::OK);
    let body = create_resp.into_body().collect().await.unwrap().to_bytes();
    let child_id = serde_json::from_slice::<serde_json::Value>(&body).unwrap()["category_id"]
        .as_str()
        .unwrap()
        .to_string();
    ...
    let add_resp = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header(axum::http::header::COOKIE, &cookie)
                .body(Body::from(format!(
                    r#"{{"url":"{feed_url}","title":"Nested Feed","category_id":"{child_id}"}}"#
                )))
                .unwrap(),
        )
        .await
...
async fn overview_returns_per_category_cards_and_all_totals() {
    ...
    let add_resp = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header(axum::http::header::COOKIE, &cookie)
                .body(Body::from(format!(
                    r#"{{"url":"{feed_url}","title":"Overview Feed"}}"#
                )))
                .unwrap(),
        )
        .await
    ...
    let toplevel = cards
        .iter()
        .find(|card| card["name"] == "NewsFlash.Toplevel")
        .expect("cards should contain a NewsFlash.Toplevel card");
    assert!(toplevel["total_count"].as_i64().unwrap() >= 2);
    let items = toplevel["items"]
        .as_array()
        .expect("card items should be an array");
