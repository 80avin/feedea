...
mod tests {
    ...
    async fn scheduler_syncs_feed() {
        ...
        let server =
            crate::engine::tests::FeedServer::start(crate::engine::tests::RSS.to_string(), 6);
        ...
        engine
            .add_feed(&server.url, Some("Sched Feed".into()), None)
            .await
            .unwrap();
        ...
        assert!(after > before);
        let unread = engine.with_nf(|nf| nf.unread_count_all()).await.unwrap();
        assert_eq!(unread, 2);
        server.stop();
    }
}
