...
impl Engine {
    ...
    pub async fn add_feed(
        ...
        let (feed, feed_mapping, _, _) = self
            .nf
            .add_feed(&url, title, category_id, &self.client)
            .await?;
        let feed_id = feed.feed_id.clone();
        if let Err(error) = self
            .nf
            .fetch_feed(&feed_id, &self.client, reqwest::header::HeaderMap::new())
            .await
        {
    ...
    pub async fn discover(&self, url: &str) -> anyhow::Result<Discovered> {
        ...
        let parsed = news_flash::feed_parser::download_and_parse_feed(
            &url,
            &id,
            None,
            semaphore,
        ...
        .await;
        match parsed {
            Ok(news_flash::feed_parser::ParsedUrl::SingleFeed(feed)) => Ok(Discovered {
                ...
                Ok(Discovered {
                    ...
                    alternatives: feeds
                        ...
                        .map(|f| {
                            (
                                f.label,
                                f.feed_url.map(|u| u.to_string()).unwrap_or_default(),
                            )
                        })
                        .collect(),
                })
    ...
    pub async fn rename_feed(&self, id: &str, title: &str) -> anyhow::Result<()> {
        let _guard = self.mutation_guard().await;
        self.nf
            .rename_feed(&FeedID::new(id), title, &self.client)
            .await?;
        Ok(())
    }
    ...
    pub async fn mark_feed_read(&self, id: &str) -> anyhow::Result<()> {
        let _guard = self.mutation_guard().await;
        self.nf
            .set_feed_read(&[FeedID::new(id)], &self.client)
            .await?;
        Ok(())
    }
    ...
    pub async fn get_feeds(&self) -> anyhow::Result<Vec<FeedSummary>> {
        ...
        for feed in feeds {
            ...
            out.push(FeedSummary {
                category_id: category_by_feed
                    .get(&id)
                    .cloned()
                    .unwrap_or_else(|| "NewsFlash.Toplevel".to_string()),
    ...
    pub async fn category_unread_map(
        &self,
        ...
        for m in &mappings {
            ...
            *direct
                .entry(m.category_id.as_str().to_string())
                .or_insert(0) += count;
        }
    ...
    pub async fn add_category(&self, name: &str, parent: Option<&str>) -> anyhow::Result<String> {
        ...
        let (category, _) = self
            .nf
            .add_category(name, parent.as_ref(), &self.client)
            .await?;
    ...
    pub async fn rename_category(&self, id: &str, name: &str) -> anyhow::Result<()> {
        let _guard = self.mutation_guard().await;
        self.nf
            .rename_category(&news_flash::models::CategoryID::new(id), name, &self.client)
            .await?;
        Ok(())
    }
    ...
    pub async fn remove_category(&self, id: &str, remove_children: bool) -> anyhow::Result<()> {
        ...
        self.nf
            .remove_category(
                &news_flash::models::CategoryID::new(id),
                remove_children,
                &self.client,
            )
            .await?;
    ...
    pub async fn mark_category_read(&self, id: &str) -> anyhow::Result<()> {
        let _guard = self.mutation_guard().await;
        self.nf
            .set_category_read(&[news_flash::models::CategoryID::new(id)], &self.client)
            .await?;
        Ok(())
    }
    ...
    pub async fn get_category_tree(&self) -> anyhow::Result<Vec<crate::dto::CategoryNode>> {
        ...
        let mut children_by_parent: HashMap<String, Vec<news_flash::models::CategoryID>> =
            HashMap::new();
    ...
    pub async fn mark_article_saved(&self, id: &str, saved: bool) -> anyhow::Result<()> {
        ...
        let marked = if saved {
            news_flash::models::Marked::Marked
        } else {
            news_flash::models::Marked::Unmarked
        };
        self.nf
            .set_article_marked(&[article_id], marked, &self.client)
            .await?;
        Ok(())
    }
    ...
    pub async fn set_article_read(&self, id: &str, read: bool) -> anyhow::Result<()> {
        ...
        let article_id = news_flash::models::ArticleID::new(id);
        let read = if read {
            news_flash::models::Read::Read
        } else {
            news_flash::models::Read::Unread
        };
        self.nf
            .set_article_read(&[article_id], read, &self.client)
            .await?;
        Ok(())
    }
    ...
    pub async fn render_article_content(
        &self,
        article: &mut crate::dto::ArticleDetail,
    ) -> anyhow::Result<()> {
    ...
    pub async fn get_favicon(&self, feed_id: &str) -> anyhow::Result<Option<(String, Vec<u8>)>> {
        let feed_id = FeedID::new(feed_id);
        let icon = self
            .with_nf(move |nf| nf.load_icon_from_db(&feed_id))
            .await?;
        let data = icon.highres.or(icon.lowres);
        Ok(data.map(|bytes| {
            (
                icon.format.unwrap_or_else(|| "image/x-icon".to_string()),
                bytes,
            )
        }))
    }

    pub async fn get_article_thumbnail(
        &self,
        article_id: &str,
        ...
        match self
            .nf
            .get_article_thumbnail(&article_id, &self.client)
            .await?
        {
            Some(thumbnail) => {
...
"#;
    ...
    async fn engine_inits_headless_with_dirs() {
        ...
        let empty = engine.with_nf(|nf| nf.is_database_empty()).await.unwrap();
        assert!(empty);
    }
    ...
    async fn add_feed_and_sync_pulls_articles() {
        ...
        let article_count = engine
            .with_nf(|nf| {
                nf.get_article_ids(news_flash::models::ArticleFilter::default())
                    .map(|v| v.len())
            })
            .await
            .unwrap();
