...
pub async fn list(State(state): State<AppState>) -> ApiResult<Json<Value>> {
    ...
    for feed in feeds {
        ...
        let category_name = name_by_id.get(&category_id).cloned().unwrap_or_else(|| category_id.clone());
        if let Some(group) = groups.iter_mut().find(|g| g.category_id == category_id) {
            group.feeds.push(feed);
        } else {
            groups.push(SourceGroup { category_id, category_name, feeds: vec![feed] });
        }
    }
...
pub async fn add(State(state): State<AppState>, Json(req): Json<AddSourceRequest>) -> ApiResult<Json<FeedSummary>> {
    ...
    let feed = match discovered.feed_url {
        Some(feed_url) => state.engine.add_feed(&feed_url, title, req.category_id).await?,
        None => state.engine.add_feed(&req.url, req.title, req.category_id).await?,
    };
    Ok(Json(feed))
}
...
pub async fn update(State(state): State<AppState>, Path(id): Path<String>, Json(req): Json<UpdateSourceRequest>) -> ApiResult<Json<FeedSummary>> {
    ...
    let feeds = state.engine.get_feeds().await?;
    let feed = feeds.into_iter().find(|f| f.id == id).ok_or_else(|| ApiError::not_found("feed not found"))?;
    Ok(Json(feed))
}
...
pub async fn import_opml(State(state): State<AppState>, Json(req): Json<ImportOpmlRequest>) -> ApiResult<Json<Value>> {
