# feedea — Phased Implementation Roadmap
...
| 1 | **Project scaffolding** | **DONE (2026-08-14).** `make dev` runs backend on :3000 serving `GET /api/health` (`{"status":"ok","version":"0.1.0"}`). App DB (`feedea.sqlite`) initializes with saved/saved_tags/tags/sessions/settings. Config/CLI parsing + logging working. Axum router wired. 6/6 tests, clippy clean, merged to main (branch `feat/phase1-scaffolding`). Plan: `docs/superpowers/plans/2026-08-14-phase1-scaffolding.md`. |
...
| 4 | **Frontend shell + PWA** | **DONE (2026-08-14).** Vite+bun+TS+HeroUI v3+Tailwind v4+React Query+React Router v8+PWA scaffold. Typed API client (DTOs match backend) + session auth + login. Desktop 3-panel (sidebar\|timeline\|reader) + mobile bottom-nav shell (CSS-only responsive). React Query data layer (overview/feeds/saved/settings + sidebar categories/sources + mutations). Debounced search suggestions + search bar + infinite-scroll sentinel. Browser-verified (headless CDP). 1 real bug found+fixed via browser check (shared query key shape). Final review fixes: id-encoding in all API hooks, feeds/* wildcard keeps timeline mounted (single /feeds/:id reader mechanism), session-error retry state. Merged to main. Plan: `docs/superpowers/plans/2026-08-14-phase4-frontend-shell.md`. Parked: apiGetText 401 bypass, redundant 401 listeners, formatAge edge, manifest duplication, h-dvh. |
| 5 | **Frontend pages** | **DONE (2026-08-14).** Feature-complete PWA per spec §7: Reader (metadata, backend-rewritten HTML w/ proxied images, Read/Save/Open/Share, note+tag editing, mobile bottom action bar + back); Overview category cards; Sources (desktop slide-over / mobile page, add-with-Fetch-title discovery, OPML import/export, per-source menu); Saved (month grouping + note/tag editing, additive `Headline.tags` backend field); Settings polish + sidebar unread badges + Help. Browser-verified per-task (headless CDP). Final review fixes: SaveDialog edit-wipe + error state, AddSource stale-URL, invalidations, 401 dispatch, mobile back. Merged to main. Plan: `docs/superpowers/plans/2026-08-14-phase5-frontend-pages.md`. Parked for Phase 6: silent mutation errors, slide-over a11y, Overview items inert, misc polish. |
| 7 | **Frontend UX polish** | **DONE (2026-08-14).** Fresh-eyes audit + all 15 user-reported UX issues fixed together: functional theme (light/dark/system) + accent colors across ALL surfaces (0 white leaks, reader buttons restyled); search bar full-width; 2-letter colored FeedAvatar (per-source color); clickable overview items; sources in the main layout (slide-over removed); sidebar add-source/add-category/import icon buttons; mobile overflow + "Exported" fixes; feed right-click + touch-only long-press context menu (Edit/Delete dialogs fixed); mobile infinite-scroll fixed; one-category sidebar fit; category creation UI; mark-read-on-open + saved notes below reader content. Smoke + all suites green; release binary re-verified. Plan: `docs/superpowers/plans/2026-08-14-phase7-ux-polish.md`. Backlog: `docs/superpowers/backlog/2026-08-14-frontend-ux-polish.md`. |
