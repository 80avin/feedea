...
|-------|------|----------------------------------|
| 0 | **Research & de-risking** | Subagent reports with evidence: (a) news-flash headless viability as server engine, (b) mapping of every API endpoint to news-flash methods + gaps, (c) frontend stack (HeroUI + Vite + bun + PWA) pinned to versions. Roadmap/plans updated with findings. |
| 1 | **Project scaffolding** | `make dev` starts backend on :3000 serving `/api/health`. App DB initializes. Config/CLI parsing + logging working. Empty axum router wired. |
