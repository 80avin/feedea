...
| 8 | **GitHub CI** | **DONE (2026-08-15).** `rust-toolchain.toml` (stable). `ci.yml` (push/PR): bun build → fmt/clippy/tests + frontend typecheck/lint. `release.yml` (tags `v*`): 5-target native matrix (linux-x64, linux-arm64, windows-x64, macos-arm64, macos-x64), smoke gate on linux-x64, binaries attached to a draft GitHub Release. Fixed mid-flight: Windows rename split into `runner.os`-gated steps (bash/pwsh), `macos-13`→`macos-15`/`macos-15-arm64` (retired labels). README CI section. Plan: `docs/superpowers/plans/2026-08-15-github-ci.md`. Deferred: bun `--frozen-lockfile`, SHA-pinned action refs. |
