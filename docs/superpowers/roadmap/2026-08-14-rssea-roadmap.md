...
| 6 | **Integration & shipping** | **DONE (2026-08-14).** Single binary: frontend `dist/` embedded via rust-embed with SPA fallback (deep links + `/api/*` + `/img` coexist on one port; build.rs guards missing dist). PWA icons + dark manifest theme. `make build` (bun→cargo release), `make dev` (concurrent backend+vite), `make test` (builds dist first), `make run`, `make clean`. README + docs/selfhosting.md (systemd unit, env vars, data-dir layout, first-run password, GPL-3.0 note). `scripts/smoke.sh` end-to-end verifies the release binary standalone (login→feed→sync→overview→saved→settings→static→SPA→manifest). Verified on merged main. Plan: `docs/superpowers/plans/2026-08-14-phase6-shipping.md`. |
